...
При переключении между коммитами гита будут меняться и отслеживаемые им `*.dvc` файлы. Для того, чтобы получить исходный файл из локального кеша, соответствующий новому `*.dvc` файлу, достаточно использовать команду
