...
Данное руководство предназначено для быстрого введения в систему версионирования данных DVC.

Многая информация намерено упрощена или опущена.

В [официальной документации DVC](https://dvc.org/doc) вы сможете найти более полное описание принципов работы системы и найти больше полезных функций и примеров.


# GitHub репозиторий

Вы можете создать репозиторий для проекта самостоятельно, но предпочтительнее, чтобы они принадлежали акккаунту кафедры @spbstu-applied-math. Для этого обратитель на кафедру, сообщите имя репозитория, который нужно создать и аккаунты, поторым нужно открыть доступ.
...
Если вы клонировали проект, который уже добавлен в систему, то вам остается лишь [добавить ключ доступа](#добавление-ключа-доступа) и сделать pull
...
Если вы изменили `model.h5` и хотите добавить новую версию файла, вызовите для него `dvc add` ещё раз.
...
При переключении между коммитами гита будут меняться и отслеживаемые им `*.dvc` файлы.
Для того, чтобы получить исходный файл из локального кеша, соответствующий новому `*.dvc` файлу, достаточно использовать команду
...
Отправляет локальный кеш DVC для имеющихся файлов `dvc.lock` и `*.dvc` в удалённое хранилище по умолчанию.
...
Загружает локальный кеш DVC для имеющихся файлов `dvc.lock` и `*.dvc` из удалённого хранилища по умолчанию.
...
Выполняет `dvc fetch` и `dvc checkout`.
...
Хотя приведённых выше инструментов достаточно для версионирования больших файлов, DVC также предлагает удобные инструменты для структурирования и репродуцирования экспериментов.

В DVC предлагается разбивать эксперимент на шаги (`stages`).
Каждый шаг харрактеризуется:
* консольной командой, которая исполняется на этом шаге
* списком зависомостей, файлов и директорий, от которых зависит результат вполнения шага (модули кода, исходные данные и данные полученные от других шагов)
* списком выходных файлов

Таким образом, из объявленых шагов формируется граф зависимостей, образуя pipeline.

Такой подход позволяет на очередные изменения запускать только затронутые ими цепочки шагов.


## `dvc stage add`

Используется для создания шага эксперимента. При добавлении флага `--force` модифицирует существующий шаг. Пример использования:

```bash
dvc stage add -n stage-fit -d fit.py -d model.h5 -d train_dataset -o trained_model.h5 "python fit.py model.h5 trained_model.h5"
```

Здесь создается шаг с именем `stage-fit` который исполняет команду `python fit.py model.h5 trained_model.h5`. Для него указаны зависимости:

* код обучения для `fit.py`
* обучаемая модель `model.h5`
* директория с датасетом для обучения `train_dataset`

Также определён выходной файл с обученной моделью `trained_model.h5`.
...
Печатает граф зависимостей шагов и данных.
...
Запускает шаги эксперимента, которые уже не актуальны, или ещё не запускались. Хеш коды входных, выходных и промежуточных файлов записывает в `dvc.lock`.
...
DVC предлагает команду [`dvc import`](https://dvc.org/doc/command-reference/import), которая позволяет импортировать файлы, отслеживаемые DVC, из других git репозиториев.

Позже можно использовать команду [`dvc update`](https://dvc.org/doc/command-reference/update) для получения новой версии файла (можно указать коммит, ветку или тег).

Поэтому возможно будет полезно создать отдельный репозиторий для датасета, если предполагается его изменение в ходе работы.
Тогда можно быстро импортировать требуемые версии в различные проекты.


## Автоматическое тестирование

Если вы настроили pipeline эксперимента и хотите автоматически проводить тестирование новых версий, то все необходимые тесты можно точно также сделать шагами pipeline.

Таким образом, при запуске всего эксперимента они будут проводиться автоматически. И наоборот, при запуске теста через dvc он убедится, что все проверяемые зависимости актуальны.